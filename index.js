...
exports.handler = function index (event, context, callback) {
  ...
  function uploadData (count, totalProjects) {
    if (count === totalProjects) {
      ...
      uploadParams.Body = s3BodyGeoJSON
      uploadParams.Key = 'allProjects-minified-v2.json'
      uploadToCloud(uploadParams)
      ...
      api.setBranch(repoBranch)
        ...
        [
          {content: JSON.stringify(projects), path: 'allProjects-minified-v2.json'},
          ...
          {content: JSON.stringify(campaigns), path: 'campaign-match.json'}
        ])
        )
        .then(function () {
